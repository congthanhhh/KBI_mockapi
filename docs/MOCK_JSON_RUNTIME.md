...
Core flow collections seed realistic PO-to-DTO scenarios with at least 9 records for item, PO, DO, shipment, customs, carrier DO, and DTO tables. `shipment_milestones` has 140 records for 14 shipments times 10 milestones. Smaller reference collections such as currencies, incoterms, transport modes, suppliers, and item groups remain compact lookup sets. Master-data reseed reads the six `docs/master_data/*.html` sheets: charge codes keep the doc's seven `group` sections and nine per-row `category` values, UOMs are limited to the 26 rows currently present in `06_UOM.html`, incoterms keep the doc set plus quotation-required `FCA`/`CFR`, and transport modes are `SEA`, `AIR`, `ROAD`, and `RAIL`. Phase-1 master-data collections include 2 `forwarders`, 3 `carriers`, and 20 `task_templates`.
